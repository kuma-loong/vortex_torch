// batch_1_id1 — warp_aggregated_histogram.
//
// Same base as k_256 winner.  **Novelty**: replace the single shared
// 256-bin histogram (whose `atomicAdd(&vh_histogram[bin], 1)` causes
// severe contention on clustered distributions where one bin gets
// hundreds of items) with **per-warp private histograms** plus a
// cross-warp reduction at the end.
//
// Memory layout: `warp_hist[NUM_WARPS][RADIX + 1]` = 8 × 257 × 4B
// = ~8KB static smem. Within a warp, 32 lanes vs 256 bins → much
// lower contention than 512 threads vs 256 bins (16x reduction).
// After the histogram pass, an 8-way warp-parallel reduction merges
// `warp_hist[w][bin]` into `vh_histogram[bin]` (one warp handles
// one quadrant of 32 bins each).
//
...
constexpr int NUM_WARPS = kThreadsPerBlock / 32;
...
__device__ void fast_topk_warpagg(
    ...
    int           target_k)
{
    ...
    // master histogram (used for cumsum + threshold detect)
    alignas(128) __shared__ int vh_histogram[RADIX + 128];
    // per-warp histograms (NUM_WARPS x RADIX) — eliminates inter-warp
    // atomic contention on the histogram pass.
    alignas(128) __shared__ int warp_hist[NUM_WARPS][RADIX];
    ...
    // Zero per-warp histograms — each thread zeroes one entry of its
    // own warp's histogram (32 threads × 8 = 256 cells per warp,
    // covered in 8 unrolled iterations).
    #pragma unroll
    for (int i = 0; i < RADIX; i += 32) {
        warp_hist[warp][i + lane] = 0;
    }
    if (tx < RADIX + 1) vh_histogram[tx] = 0;
    __syncthreads();

    // --- Histogram pass: each thread updates its own warp's histogram.
    for (int i = tx; i < head_count; i += BLOCK_SIZE) {
        const float f = vortex_to_float(input[row_start + i]);
        ::atomicAdd(&warp_hist[warp][convert_to_uint8(f)], 1);
    }
    #pragma unroll
    for (int it = 0; it < MAX_ITERS; ++it) {
        ...
        if (q < quads) {
            ...
            reg_packed_bins[it] = uint32_t(b0)
                                ...
                                | (uint32_t(b2) << 16)
                                | (uint32_t(b3) << 24);
            ::atomicAdd(&warp_hist[warp][b0], 1);
            ::atomicAdd(&warp_hist[warp][b1], 1);
            ::atomicAdd(&warp_hist[warp][b2], 1);
            ::atomicAdd(&warp_hist[warp][b3], 1);
        }
    }
    ...
    // --- Cross-warp reduction: each lane sums its bin across all warps.
    // tx ∈ [0, RADIX) — direct mapping; threads beyond RADIX idle here.
    if (tx < RADIX) {
        int s = 0;
        #pragma unroll
        for (int w = 0; w < NUM_WARPS; ++w) s += warp_hist[w][tx];
        vh_histogram[tx] = s;
    }
    if (tx == RADIX) vh_histogram[RADIX] = 0;
    __syncthreads();
    ...
    const int     page_reserved_eos)
{
    ...
    if constexpr (std::is_same_v<ScoreT, __nv_bfloat16>) {
        fast_topk_warpagg<ScoreT, /*MaxRefineRounds=*/2>(score, s_indices, start, nblk, topk_val);
    } else {
        fast_topk_warpagg<ScoreT, /*MaxRefineRounds=*/4>(score, s_indices, start, nblk, topk_val);
    }
    __syncthreads();
