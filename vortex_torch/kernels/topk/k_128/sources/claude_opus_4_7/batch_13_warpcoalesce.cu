    ...
    int           target_k)
{
    ...
    // ----- ADAPTIVE FAST PATH (WARP-COALESCED EMIT) -----
    // Reduces 32x the atomic contention on vh_counter / vh_last_remain
    // by aggregating per-warp emits via __ballot_sync + lane-0 atomicAdd.
    if (last_remain <= TOLERATE_THRESH) {
        const int lane_local = tx & 31;
        const unsigned ACTIVE = 0xffffffffu;

        // Helper lambda emits one element via warp-aggregated atomics.
        auto warp_emit = [&] (bool valid, int bin, int idx) {
            const bool is_strict = valid && (bin > initial_threshold_bin);
            const bool is_threshold = valid && (bin == initial_threshold_bin);

            const unsigned strict_mask = __ballot_sync(ACTIVE, is_strict);
            const int strict_count = __popc(strict_mask);
            const int my_strict_offset = __popc(strict_mask & ((1u << lane_local) - 1));
            int warp_strict_base = 0;
            if (lane_local == 0 && strict_count > 0) {
                warp_strict_base = ::atomicAdd(&vh_counter, strict_count);
            }
            warp_strict_base = __shfl_sync(ACTIVE, warp_strict_base, 0);
            if (is_strict) {
                index[warp_strict_base + my_strict_offset] = idx;
            }

            const unsigned threshold_mask = __ballot_sync(ACTIVE, is_threshold);
            const int threshold_count = __popc(threshold_mask);
            if (threshold_count == 0) return;
            const int my_thr_offset = __popc(threshold_mask & ((1u << lane_local) - 1));
            int warp_remain_base = 0;
            if (lane_local == 0) {
                warp_remain_base = ::atomicAdd(&vh_last_remain, -threshold_count);
            }
            warp_remain_base = __shfl_sync(ACTIVE, warp_remain_base, 0);
            if (is_threshold) {
                const int pos = warp_remain_base - my_thr_offset;
                if (pos > 0) {
        ...
        // Head — scalar; loops over BLOCK_SIZE-aligned chunks so each warp
        // converges. Use valid=false padding when i >= head_count.
        {
            const int max_iters_head = (head_count + BLOCK_SIZE - 1) / BLOCK_SIZE;
            for (int it = 0; it < max_iters_head; ++it) {
                const int i = it * BLOCK_SIZE + tx;
                const bool valid = (i < head_count);
                int bin = -1;
                if (valid) bin = int(convert_to_uint8(vortex_to_float(input[row_start + i])));
                warp_emit(valid, bin, i);
            }
        }

        // Quad pass — register-cached bins, 4-element per thread per iter.
        #pragma unroll
        for (int it = 0; it < MAX_ITERS; ++it) {
            const int q = tx + it * BLOCK_SIZE;
            const bool valid_q = (q < quads);
            const uint32_t packed = valid_q ? reg_packed_bins[it] : 0u;
            const int base = head_count + 4 * q;
            #pragma unroll
            for (int k = 0; k < 4; ++k) {
                const int bin = valid_q ? int((packed >> (k * 8)) & 0xFF) : -1;
                warp_emit(valid_q, bin, base + k);
            }
        }

        // Tail — scalar, same warp-converged padding loop.
        {
            const int max_iters_tail = (tail_count + BLOCK_SIZE - 1) / BLOCK_SIZE;
            for (int it = 0; it < max_iters_tail; ++it) {
                const int i = it * BLOCK_SIZE + tx;
                const bool valid = (i < tail_count);
                int bin = -1;
                int idx = -1;
                if (valid) {
                    idx = tail_start + i;
                    bin = int(convert_to_uint8(vortex_to_float(input[row_start + idx])));
                }
                warp_emit(valid, bin, idx);
            }
        }
        __syncthreads();
        return;
