    ...
    for (int it = 0; it < MAX_ITERS; ++it) {
        ...
        if (q < quads) {
            ...
            reg_packed_bins[it] = uint32_t(b0)
                                ...
                                | (uint32_t(b2) << 16)
                                | (uint32_t(b3) << 24);
            // Per-thread atomic aggregation: when consecutive bins in a
            // quad are the same (common on clustered distributions),
            // batch them into one atomicAdd with count. For uniform
            // distributions all 4 differ → 4 separate atomicAdds.
            ...
            ::atomicAdd(&vh_histogram[b0], c0);
            if (c1) ::atomicAdd(&vh_histogram[b1], c1);
            if (c2) ::atomicAdd(&vh_histogram[b2], c2);
            if (c3) ::atomicAdd(&vh_histogram[b3], c3);
        }
    }
