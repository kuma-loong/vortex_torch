    ...
    for (int it = 0; it < MAX_ITERS; ++it) {
        ...
        if (q < quads) {
            ...
            reg_packed_bins[it] = uint32_t(b0)
                                ...
                                | (uint32_t(b2) << 16)
                                | (uint32_t(b3) << 24);
            // Warp-aggregated atomic: lanes with the same bin coalesce
            // into a single atomicAdd by the leader lane (lowest active
            // lane matching this bin). Reduces global atomic traffic
            // proportional to bin-equality within each warp.
            const unsigned mask_w = 0xffffffffu;
            const int lane_id = threadIdx.x & 31;
            #pragma unroll
            for (int k = 0; k < 4; ++k) {
                uint8_t b = (k == 0) ? b0 : (k == 1) ? b1 : (k == 2) ? b2 : b3;
                unsigned peers = __match_any_sync(mask_w, (uint32_t)b);
                int leader = __ffs(peers) - 1;
                if (lane_id == leader) {
                    ::atomicAdd(&vh_histogram[b], __popc(peers));
                }
            }
        }
    }
