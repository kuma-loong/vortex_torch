// batch_0_id1 — bf16_direct_resequenced.
//
// Same base as the k_256 winner (regcache_unrolled + alignment-safe vec4 +
// warp-shuffle cumsum + monotonic transform).  **Novelty**: drop the
// fp16-detoured initial bin (`__float2half_rn` → top byte) in favor of a
// **direct bf16 → 8-bit radix bin** that maintains correctness by
// **re-sequencing the refinement byte schedule**.
//
// k_256 batch_5_id1 broke recall (R@253 = 0.96) because it kept the
// refinement schedule at offsets {24, 16, 8} but replaced initial with
// bf16-direct (= fp32 byte 3 = bf16 top byte) — so refinement round-0
// at offset=24 became a no-op redundant with the initial bin.  Fix:
// **start refinement at offset=16** (bf16 low byte = the next meaningful
// byte for bf16 inputs).  Only one meaningful refinement remains for
// bf16; the last-useful round uses atomic-arrival to break ties.
//
// Win vector: removes the bf16→fp32→fp16→top-byte conversion in both
// the round-0 histogram and round-0 filter passes (~3-5 cycles/element).
//
...
// Direct bf16 → 8-bit radix bin. Sign-flip preserves monotonic order
// (negatives invert, positives flip MSB). Cheaper than the fp16-detour.
__device__ __forceinline__ uint8_t bf16_to_uint8(float f) {
    // f is already transformed; re-cast to bf16 then to bits.
...
// 16-bit bf16 radix key (used for refinement byte extraction). Returns
// the same monotonic-sign-flipped uint16, which we treat as a 2-byte
// big-endian word: byte 1 = top (initial bin), byte 0 = low (refinement).
...
// Bin extraction by dtype: bf16-direct for bf16, fp16-detour for float
// (rare in the benchmark — keep the safe path).
...
// Refinement byte sequence depends on dtype. For bf16, only the bf16 low
// byte carries new information; we start at offset=8 of the 16-bit key
// (i.e., byte 0 of the uint16 key).
template <typename ScoreT, int Round>
__device__ __forceinline__ int refinement_bin(float f);
template <> __device__ __forceinline__ int refinement_bin<__nv_bfloat16, 0>(float f) {
    // Byte 0 of the 16-bit bf16 key (bf16 low byte after sign flip).
    return bf16_to_uint16_key(f) & 0xFF;
}
template <int Round>
__device__ __forceinline__ int refinement_bin_float(float f) {
    // For fp32 paths: offset 16, 8, 0 of the 32-bit key (bytes 2, 1, 0).
...
__device__ void fast_topk_bf16direct(
    ...
    int           target_k)
{
    ...
    // --- Histogram pass (initial = bf16 top byte; cheaper than fp16-detour) ---
    ...
    // --- Round-0 filter + sub-histogram on bf16 low byte (refinement byte). ---
    auto handle_scalar = [&](float f, int idx) {
        ...
        if (bin > initial_threshold_bin) {
            ...
            if (C10_LIKELY(pos < SMEM_INPUT_SIZE)) {
                ...
                // Refinement byte: bf16 low byte (no fp32 detour for bf16).
                int sub_bin;
                if constexpr (std::is_same_v<ScoreT, __nv_bfloat16>) {
                    sub_bin = refinement_bin<ScoreT, 0>(f);
                } else {
                    sub_bin = refinement_bin_float<0>(f);
                }
                ...
                for (int k = 0; k < 4; ++k) {
                    if (bins[k] == initial_threshold_bin) {
                        ...
                        if (C10_LIKELY(pos < SMEM_INPUT_SIZE)) {
                            ...
                            int sub_bin;
                            if constexpr (std::is_same_v<ScoreT, __nv_bfloat16>) {
                                sub_bin = refinement_bin<ScoreT, 0>(fs[k]);
                            } else {
                                sub_bin = refinement_bin_float<0>(fs[k]);
                            }
                            ::atomicAdd(&vh_histogram[sub_bin], 1);
                        }
                    }
    ...
    for (int round = 0; round < MaxRefineRounds; ++round) {
        ...
        if (topk == 0) {
            for (int i = tx; i < num_input; i += BLOCK_SIZE) {
                ...
                int bin;
                if constexpr (std::is_same_v<ScoreT, __nv_bfloat16>) {
                    bin = refinement_bin<ScoreT, 0>(f);
                } else {
                    bin = refinement_bin_float<0>(f);
                }
        ...
        // For bf16 we never get here (MaxRefineRounds=1). For float we'd
        // do another sub_bin pass — but this kernel is optimised for the
        // bf16 path; just keep functional correctness.
        __syncthreads();
        if (tx < RADIX + 1) vh_histogram[tx] = 0;
        __syncthreads();
        for (int i = tx; i < num_input; i += BLOCK_SIZE) {
            ...
            if (bin > threshold_bin) {
                ...
                if (C10_LIKELY(pos < SMEM_INPUT_SIZE)) {
                    ...
                    // No further refinement byte exists for bf16; just
                    // bucket into bin 0 so atomic-arrival finishes us off.
    ...
    const int     page_reserved_eos)
{
    ...
    // bf16: 1 meaningful refinement round (bf16 low byte). float: 2 (bytes 2, 1).
    if constexpr (std::is_same_v<ScoreT, __nv_bfloat16>) {
        fast_topk_bf16direct<ScoreT, /*MaxRefineRounds=*/1>(score, s_indices, start, nblk, topk_val);
    } else {
        fast_topk_bf16direct<ScoreT, /*MaxRefineRounds=*/2>(score, s_indices, start, nblk, topk_val);
    }
    __syncthreads();
