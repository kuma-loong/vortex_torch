// batch_4_id0 — adaptive_with_bin_size_safety.
//
// Same as batch_1_id0 (adaptive_approx_skip) but with an additional
// recall-safety check on the threshold-bin size: the fast-path triggers
// only when `last_remain <= TOLERATE_THRESH` **AND**
// `threshold_bin_count <= BIN_SAFETY_FACTOR * last_remain`.
//
// Rationale: atomic-arrival picks `last_remain` items from
// `threshold_bin_count` candidates. When the bin is small
// (<= BIN_SAFETY_FACTOR * last_remain), expected recall = strict
// winners + ~last_remain/BIN_SAFETY_FACTOR + ties → high.
// When the bin is large, the fast-path declines and the standard
// refinement path runs, preserving recall.
//
// Compile-time knobs: __THREADS_PER_BLOCK__ __VORTEX_MAX_TOPK__
//                     __SMEM_BYTES__ __TRANSFORM_TYPE__
//                     __TOLERATE_THRESH__ __BIN_SAFETY_FACTOR__
...
constexpr int BIN_SAFETY_FACTOR = __BIN_SAFETY_FACTOR__;
    ...
    int           target_k)
{
    ...
    // ----- ADAPTIVE FAST PATH (with bin-size safety check) -----
    // Trigger only when last_remain is small AND the threshold bin
    // isn't so wide that atomic-arrival picks a tiny random subset.
    const int threshold_bin_count =
        vh_histogram[initial_threshold_bin] - vh_histogram[initial_threshold_bin + 1];
