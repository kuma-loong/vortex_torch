// Warp-coalesced filter emit + vec4 + warp-shuffle cumsum radix top-k.
// Builds on batch_5_id0 (vec4 + warp-shuffle) by replacing per-element
// atomicAdd on vh_counter with a per-warp atomicAdd: each lane accumulates
// its winning indices locally, the warp does a prefix-scan of per-lane
// counts to compute write offsets, lane 0 grabs the warp's slice of the
// output via a single atomicAdd, and each winning element is written
// scatterless at base + warp_offset.
//
// Cuts atomic contention on vh_counter from ~K per kernel to ~K/32.
//
...
__device__ __forceinline__ void load_quad<__nv_bfloat16>(const __nv_bfloat16* p, float& f0, float& f1, float& f2, float& f3) {
    ...
    lo = *reinterpret_cast<__nv_bfloat162*>(&packed.x);
    hi = *reinterpret_cast<__nv_bfloat162*>(&packed.y);
...
// Inclusive warp prefix sum on a single int. Returns ( inclusive[lane],  warp_total ).
__device__ __forceinline__ void warp_inclusive_scan(int v, int& inclusive, int& total) {
    ...
    inclusive = v;
    total = __shfl_sync(0xffffffffu, v, 31);
}

template <typename ScoreT>
__device__ void fast_topk_warpcoal(
    ...
    int           target_k)
{
    ...
    if (topk == 0) {
        // No threshold-bin candidates needed; collect strict winners with warp-coalesced emit.
        for (int q = tx; q < quads; q += BLOCK_SIZE) {
            float f0, f1, f2, f3;
            load_quad<ScoreT>(input + row_start + 4 * q, f0, f1, f2, f3);
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                if (convert_to_uint8(f) > threshold_bin) {
                    my_winners[my_count++] = k;
                }
            }

            int incl, total;
            warp_inclusive_scan(my_count, incl, total);
            int base = 0;
            if (lane == 0) base = ::atomicAdd(&vh_counter, total);
            base = __shfl_sync(0xffffffffu, base, 0);
            int warp_offset = incl - my_count;

            #pragma unroll
            for (int w = 0; w < 4; ++w) {
                if (w < my_count) {
                    index[base + warp_offset + w] = 4 * q + my_winners[w];
                }
            }
        }
        // Tail (length not multiple of 4) — scalar.
        if (tx < tail_count) {
            const int idx = tail_start + tx;
            if (convert_to_uint8(vortex_to_float(input[row_start + idx])) > threshold_bin) {
    ...
    // Round 0 filter — warp-coalesced strict emit; per-thread threshold-bin candidate buffer fill.
    for (int q = tx; q < quads; q += BLOCK_SIZE) {
        float f0, f1, f2, f3;
        load_quad<ScoreT>(input + row_start + 4 * q, f0, f1, f2, f3);
        ...
        // Threshold-bin candidates — per-thread atomic (low contention typically).
        #pragma unroll
        for (int k = 0; k < 4; ++k) {
            const int bin = static_cast<int>(convert_to_uint8(fs[k]));
            if (bin == threshold_bin) {
    ...
    const int     page_reserved_eos)
{
    ...
    fast_topk_warpcoal<ScoreT>(score_blk, s_indices, 0, nblk, topk_val);
    __syncthreads();
