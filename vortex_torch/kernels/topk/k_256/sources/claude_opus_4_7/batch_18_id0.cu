...
//
// SMEM-CACHED: pre-loads all candidate (score, idx) pairs into shared
// memory once, then runs all subsequent radix passes against smem-resident
// scores. Eliminates the per-pass indirect global loads.
// =====================================================================
template <typename ScoreT, int MaxRefineRounds, int MAX_CANDS>
__device__ void fast_topk_subset(
    const ScoreT* __restrict__ score_seg,
    ...
    int           target_k)
{
    ...
    // Pre-load buffer for candidate (score, idx) pairs.
    __shared__ float s_cand_score[MAX_CANDS];
    __shared__ int   s_cand_idx[MAX_CANDS];

    extern __shared__ int vh_input_idx[][SMEM_INPUT_SIZE];

    const int tx = threadIdx.x;

    // Pre-load all candidate scores/indices into smem once.
    for (int i = tx; i < MAX_CANDS; i += BLOCK_SIZE) {
        if (i < num_cands) {
            const int seg_rel = cand_indices[i];
            if (seg_rel < 0) {
                s_cand_score[i] = -CUDART_INF_F;
                s_cand_idx[i] = -1;
            } else {
                s_cand_score[i] = vortex_to_float(score_seg[seg_rel]);
                s_cand_idx[i] = seg_rel;
            }
        } else {
            s_cand_score[i] = -CUDART_INF_F;
            s_cand_idx[i] = -1;
    ...
    // Round 0 histogram — read scores from smem.
    for (int i = tx; i < MAX_CANDS; i += BLOCK_SIZE) {
        const float f = s_cand_score[i];
        if (s_cand_idx[i] < 0) continue;
        ::atomicAdd(&vh_histogram[convert_to_uint8(f)], 1);
    }
    __syncthreads();
    ...
    if (topk == 0) {
        for (int i = tx; i < MAX_CANDS; i += BLOCK_SIZE) {
            const int seg_rel = s_cand_idx[i];
            if (seg_rel < 0) continue;
            if (convert_to_uint8(s_cand_score[i]) > initial_threshold_bin) {
                const int pos = ::atomicAdd(&vh_counter, 1);
                index[pos] = seg_rel;
            }
        }
    ...
    for (int i = tx; i < MAX_CANDS; i += BLOCK_SIZE) {
        ...
        if (bin > initial_threshold_bin) {
            ...
            const int pos = ::atomicAdd(&vh_num_input[0], 1);
            if (C10_LIKELY(pos < SMEM_INPUT_SIZE)) {
                // Store the LOCAL i (smem index) for refinement passes.
                vh_input_idx[0][pos] = i;
                const auto b32 = convert_to_uint32(f);
                const auto sub_bin = (b32 >> 24) & 0xFF;
                ::atomicAdd(&vh_histogram[sub_bin], 1);
            }
...
#pragma unroll
    for (int round = 0; round < MaxRefineRounds; ++round) {
        ...
        if (topk == 0) {
            // vh_input_idx[r_idx][i] holds smem-index `i_local` into s_cand_*.
            for (int i = tx; i < num_input; i += BLOCK_SIZE) {
                const int i_local = vh_input_idx[r_idx][i];
                const float raw = s_cand_score[i_local];
                const auto offset = 24 - round * 8;
                const auto bin = (convert_to_uint32(raw) >> offset) & 0xFF;
                if (bin > threshold_bin) {
                    const int pos = ::atomicAdd(&vh_counter, 1);
                    index[pos] = s_cand_idx[i_local];
                }
            }
            __syncthreads();
            return;
        ...
        if (is_last_useful_round) {
            ...
            for (int i = tx; i < num_input; i += BLOCK_SIZE) {
                ...
                if (bin > threshold_bin) {
                    ...
                    const int pos = ::atomicAdd(&vh_last_remain, -1);
                    if (pos > 0) {
                        index[target_k - pos] = s_cand_idx[i_local];
                    }
                }
            }
            __syncthreads();
        ...
        for (int i = tx; i < num_input; i += BLOCK_SIZE) {
            ...
            if (bin > threshold_bin) {
                ...
                const int pos = ::atomicAdd(&vh_num_input[r_idx ^ 1], 1);
                if (C10_LIKELY(pos < SMEM_INPUT_SIZE)) {
                    vh_input_idx[r_idx ^ 1][pos] = i_local;
                    const auto b32 = convert_to_uint32(raw);
                    const auto sub_bin = (b32 >> (offset - 8)) & 0xFF;
                    ::atomicAdd(&vh_histogram[sub_bin], 1);
                }
            }
    ...
    const int     num_splits)
{
    ...
    // Phase 2 — this CTA is the last arrival for the segment. Do merge.
    const int num_cands_total = num_splits * VORTEX_MAX_TOPK;
    const int* all_cands = workspace + seg * num_splits * VORTEX_MAX_TOPK;
    // (Includes -1 sentinels; fast_topk_subset filters them out.)

    __shared__ int s_final[VORTEX_MAX_TOPK];
    // MAX_CANDS = NUM_SPLITS_CAP * VORTEX_MAX_TOPK (compile-time upper bound)
    fast_topk_subset<ScoreT, MaxRefineRounds, NUM_SPLITS_CAP * VORTEX_MAX_TOPK>(
        score + seg_start, all_cands, num_cands_total, s_final, target_k);
    __syncthreads();

    // Write result via dense_kv_indices remap.
    int* out_blk = sparse_kv_indices + sparse_kv_indptr[seg] + page_reserved_bos;
    const int* idx_blk = dense_kv_indices + seg_start;
    for (int i = tx; i < target_k; i += kThreadsPerBlock) {
        out_blk[i] = idx_blk[s_final[i]];
    }

    // Reset done_counter[seg] to 0 so the cached buffer is ready for the
    // next topk() call without a host-side memset.
    if (tx == 0) {
        done_counter[seg] = 0;
    }
}
