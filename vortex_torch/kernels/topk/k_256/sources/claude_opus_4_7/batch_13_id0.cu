    ...
    const int     num_splits)
{
    ...
    fast_topk_subset<ScoreT, MaxRefineRounds>(
    ...
    const int64_t     max_num_pages)
{
    ...
    if (bs > SPLIT_THRESHOLD) {
        ...
        int num_splits = (256 + bs - 1) / bs;
        if (num_splits > MAX_SPLITS) num_splits = MAX_SPLITS;
        if (num_splits < 1) num_splits = 1;

        // Allocate workspace + done counter.
        auto opts_int = dense_kv_indptr.options().dtype(at::kInt);
        auto workspace = at::empty({bs * num_splits * VORTEX_MAX_TOPK}, opts_int);
        auto done_counter = at::zeros({bs}, opts_int);
