// Vec4-load (int2 = 4 bf16) + warp-shuffle cumsum radix top-k. Builds on
// batch_4_id1 by widening the round-0 load from 4 bytes (`__nv_bfloat162`)
// to 8 bytes (`int2`) — each thread processes 4 bf16 elements per load
// instead of 2. Halves the load-instruction count again at seq_len >= 2048
// (where threads actually have ≥1 int2-load worth of work).
//
// Alignment: row_start = bx * seq_len, seq_len ∈ {1024,1536,2048,4096}
// → row_start * sizeof(bf16) is a multiple of 8. Length is a multiple of 4.
//
...
// Load 4 scores into f[0..3]. Specialised for bf16 (vec4 via int2) and float (scalar 4-pack).
...
template <typename ScoreT>
__device__ void fast_topk_vec4(
    ...
    int           target_k)
{
    ...
    const int tx = threadIdx.x;
    const int quads = length >> 2;          // # of int2 loads
    const int tail_start = quads << 2;
    const int tail_count = length - tail_start;  // 0..3 leftover bf16

    if (tx < RADIX + 1) vh_histogram[tx] = 0;
    __syncthreads();

    // Round 0 histogram — vec4 loads.
    ...
    // Tail (length not divisible by 4 — does not happen on this benchmark but kept for safety).
    if (tx < tail_count) {
        const auto b = convert_to_uint8(vortex_to_float(input[row_start + tail_start + tx]));
    ...
    if (topk == 0) {
        ...
        if (tx < tail_count) {
            ...
            const auto b = convert_to_uint8(vortex_to_float(input[row_start + idx]));
            if (b > threshold_bin) {
                const auto pos = ::atomicAdd(&vh_counter, 1);
                index[pos] = idx;
            }
        ...
        if (tx < tail_count) {
            const int idx = tail_start + tx;
            const auto f = vortex_to_float(input[row_start + idx]);
            const int bin = static_cast<int>(convert_to_uint8(f));
            if (bin > threshold_bin) {
                const auto pos = ::atomicAdd(&vh_counter, 1);
                index[pos] = idx;
    ...
    const int     page_reserved_eos)
{
    ...
    fast_topk_vec4<ScoreT>(score_blk, s_indices, 0, nblk, topk_val);
    __syncthreads();
