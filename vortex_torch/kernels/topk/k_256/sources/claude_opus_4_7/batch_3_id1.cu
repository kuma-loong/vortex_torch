// Smem-cached + warp-shuffle cumsum radix top-k. Combines batch_2_id0's
// score-cache optimisation with batch_2_id1's warp-cooperative suffix-sum.
// Both wins were orthogonal at 1024 threads; this proposal tests whether
// they still compose at lower occupancy (typically T=512, smem=16KB).
//
...
// Inclusive suffix-sum on 256 ints in shared memory. Warp-cooperative;
// uses 8 active warps (lanes 0..255). Caller must have at least 256 threads.
...
__device__ void fast_topk_cached_warpscan(
    const ScoreT* __restrict__ input,
    ScoreT*       __restrict__ s_scores,
    ...
    int           target_k)
{
    ...
    // Phase 0 — cache scores into shared memory once.
    ...
    const int     page_reserved_eos)
{
    ...
    fast_topk_cached_warpscan<ScoreT>(score_blk, s_scores, s_indices, 0, nblk, topk_val);
    __syncthreads();
