// Radix top-k: BOS/EOS-aware segmented 8-bit radix selection with index remap.
//
...
// Dynamic shared memory budget (bytes). Sizes the two-bank `vh_input_idx`
// candidate buffer: SMEM_INPUT_SIZE = kSmem / (2 * sizeof(int)) entries per bank.
