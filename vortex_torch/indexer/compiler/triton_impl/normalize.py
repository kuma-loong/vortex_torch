...
from ...scan import Normalize
def generate_normalize_impl(graph: Graph, op_id: int, ctx: Context) -> str:
    ...
    assert issubclass(op.__class__, Normalize), f"Expected a normalize op, got {graph.op_list[op_id]}"
    assert t_i._format == FORMAT.RAGGED, f"Expected ragged input tensor for normalize, got {t_i._format}"
    assert t_o._format == FORMAT.RAGGED, f"Expected ragged output tensor for normalize, got {t_o._format}"

    func_def_lines = [
    f"@triton.jit",
    f"def normalize_kernel(",
    f"{INDENT}x,",
    f"{INDENT}out,",
    f"{INDENT}indptr,",
    ...
    f"{INDENT}eps: tl.constexpr = 1e-12,",
    f"):",
    ...
    f"",
    f"{INDENT}square_norm = tl.zeros((x_D0, x_D1), dtype=tl.float32)",
    f"{INDENT}eps_mat = tl.full((x_D0, x_D1), value=eps, dtype=tl.float32)",
    f"",
    ...
    f"{INDENT*2}slab = tl.load(x_base_ptr + offs, mask=mask, other=0.0).to(tl.float32)",
    f"{INDENT*2}square_norm_c = tl.sum(slab * slab, axis=0)",
    f"{INDENT*2}square_norm = square_norm + square_norm_c",
    f"",
    f"{INDENT}norm = tl.maximum(tl.sqrt(square_norm), eps_mat)",
    f"",
    f"{INDENT}for p in range(0, num_pages_to_compute, BLOCK_P):",
    f"{INDENT*2}kp = tl.minimum(BLOCK_P, num_pages_to_compute - p)",
    f"{INDENT*2}p_mask = p_idx < kp",
    ...
    f"{INDENT*2}mask = p_mask[:, None, None]",
    f"{INDENT*2}slab = tl.load(x_base_ptr + offs, mask=mask, other=0.0).to(tl.float32)",
    f"{INDENT*2}slab = slab / norm[None, :, :]",
    ...
    f"",
    f"{INDENT}normalize_kernel[(eff_batch_size,)](",
