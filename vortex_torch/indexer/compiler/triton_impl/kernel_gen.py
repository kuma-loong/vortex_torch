...
def generate_load_tensor_str(sub_graph: Graph, ctx: Context) -> str:
    ...
    for local_tensor_id in sub_graph.input_tensor_ids:
        ...
        if t._format == FORMAT.BATCHED:
            ...
            batched_lines.append(f"tensor_{local_tensor_id}_block = tl.reshape(tl.load(tensor_{local_tensor_id}_block_ptr, boundary_check=(0, 1), padding_option=\"zero\", cache_modifier=\".ca\"), (1, {t.shape[1]}, {t.shape[2]})).to(tl.float32)")
            ...
            if ctx.num_pages_per_workload == 1:
                ...
                paged_lines.append(f"tensor_{local_tensor_id}_block = tl.reshape(tl.load(tensor_{local_tensor_id}_block_ptr, boundary_check=(0, 1), padding_option=\"zero\", cache_modifier=\".cv\"), ({ctx.workload_chunk_size}, {t.shape[1]}, {t.shape[2]})).to(tl.float32)")
            else:
                ...
                paged_lines.append(f"tensor_{local_tensor_id}_block = tl.reshape(tl.load(tensor_{local_tensor_id}_block_ptr, mask=page_valid[:, None], other=0.0, cache_modifier=\".cv\"), ({ctx.workload_chunk_size}, {t.shape[1]}, {t.shape[2]})).to(tl.float32)")
