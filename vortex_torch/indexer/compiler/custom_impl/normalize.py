"""normalize — launcher emitter (Schedule.S, backend-agnostic).

Kernel body lives in ``vortex_torch.custom_ops.normalize.<backend>.default.kernel``.
...
def generate_normalize_impl(graph: Graph, op_id: int, ctx: Context) -> str:
    ...
    bk = get_backend(ctx)
    callable_name = f"_vortex_normalize_kernel_{op_id}"
    ctx.compilation_header_lines.extend([
        "from vortex_torch.custom_ops import find as _vortex_custom_ops_find",
        f"{callable_name} = _vortex_custom_ops_find('normalize', '{bk.name}')()",
    ])
