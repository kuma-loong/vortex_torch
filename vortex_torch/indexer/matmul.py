...
class GeMM(vOp):
    ...
    def profile(self, x: vTensor, y: vTensor, ctx: Context) -> vTensor:
        # Inputs must be vTensor per the system rule (no mixing with plain tensors).
        assert isinstance(x, vTensor), f"GeMM.profile expects x to be vTensor, got {type(x)}"
        assert isinstance(y, vTensor), f"GeMM.profile expects y to be vTensor, got {type(y)}"

        # Dimensionality checks (expect [*, 1, *] with both tensors being 3D).
        assert len(x.shape) == len(y.shape) == 3, (
            f"Expected 3D inputs (B/S, G/C, D). Got x.ndim={x.dim()}, y.ndim={y.dim()}"
        )
    ...
    def execute(self, x: torch.Tensor, y: torch.Tensor, ctx: Context) -> torch.Tensor:
        # Must call profile() first to set impl and output buffer.
        assert self.impl is not None, "GeMM.execute called before profile(): impl is None"
        assert self.output_buffer is not None, "GeMM.execute called before profile(): output_buffer is None"
