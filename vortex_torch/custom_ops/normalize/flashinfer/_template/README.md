# `_template/` — copy-and-edit scaffold for a new `normalize` / `flashinfer` bucket
...
Drop this whole directory under a new bucket name, then edit:
  ...
  1. **`meta.json`**
     ...
     * Replace the `constraints` placeholder with real applicability
       ...
       See `custom_ops/normalize/SPEC.md` for the kwargs this op routes on.
...
from vortex_torch.custom_ops.normalize._reference import reference_flashinfer

fn = find("normalize", "flashinfer", <your routing kwargs>)()  # picks your leaf
out_kernel = ...
out_ref    = ...
