# `_template/` — copy-and-edit scaffold for a new `conv_1d` / `flashinfer` bucket
...
Drop this whole directory under a new bucket name, then edit:
  ...
  1. **`meta.json`**
     ...
     * Replace the `constraints` placeholder with real applicability
       ...
       See `custom_ops/conv_1d/SPEC.md` for the kwargs this op routes on.
...
from vortex_torch.custom_ops.conv_1d._reference import reference_flashinfer

fn = find("conv_1d", "flashinfer", <your routing kwargs>)()  # picks your leaf
out_kernel = ...
out_ref    = ...
