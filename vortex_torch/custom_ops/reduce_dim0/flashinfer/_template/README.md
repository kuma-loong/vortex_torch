# `_template/` — copy-and-edit scaffold for a new `reduce_dim0` / `flashinfer` bucket
...
Drop this whole directory under a new bucket name, then edit:
  ...
  1. **`meta.json`**
     ...
     * Replace the `constraints` placeholder with real applicability
       ...
       See `custom_ops/reduce_dim0/SPEC.md` for the kwargs this op routes on.
     * Bump `priority` if you want to beat siblings at the same
       specificity level.

  2. **`kernel.py` / `config.json`** (CUDA) or **`kernel.py`** (Triton)
  ...
  3. **`dispatch.py`**
     * Usually no edit needed. The boilerplate wires `kernel.py`
...
from vortex_torch.custom_ops.reduce_dim0._reference import reference_flashinfer

fn = find("reduce_dim0", "flashinfer", <your routing kwargs>)()  # picks your leaf
out_kernel = ...
out_ref    = ...
fn(*args)                              # writes out_kernel
reference_flashinfer(*ref_args)         # writes out_ref
