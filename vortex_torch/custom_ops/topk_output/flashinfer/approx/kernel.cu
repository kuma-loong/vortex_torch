// Approximate top-k: single-pass 8-bit radix selection with an optional
// second pass when the threshold bin is too wide.
//
...
//      Pass 3 — items with bin0 == tbin0 && bin1 > tbin1 are strict
//               winners, items with bin0 == tbin0 && bin1 == tbin1
//               fill the remaining slots in atomic-arrival order.
//
//   tolerate_ratio = 1.0 → always single-pass (cheapest, loosest).
//   tolerate_ratio = 0.0 → always two-pass (tighter approximation).
//
// Compile-time knobs (substituted by kernels/topk/dispatcher.py before nvcc):
//   __THREADS_PER_BLOCK__  — CTA width (multiple of 32, in [256, 1024])
    ...
    const int     tolerate_thresh)
{
    ...
    for (int idx = tx; idx < length; idx += BLOCK_SIZE) {
        const auto bin =
            (score_to_key32(vortex_to_float<ScoreT>(input[idx])) >> 24) & 0xFFu;
    ...
    for (int idx = tx; idx < length; idx += BLOCK_SIZE) {
        const auto key32 = score_to_key32(vortex_to_float<ScoreT>(input[idx]));
    ...
    const int     block_reserved_eos)
{
    ...
    int tolerate_thresh =
        static_cast<int>(kTolerateRatio * static_cast<float>(target_k));
