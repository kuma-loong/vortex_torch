// Register bin-cache (UNROLLED with compile-time-constant indices to avoid
// the spill that broke batch_10_id0) + alignment-safe vec4 + bf16-aware
// explicit-skip + warp-shuffle cumsum.
//
// Key fix vs batch_10_id0: the histogram and filter loops are written as
// fixed-trip `for (int it = 0; it < MAX_ITERS; ++it)` with `#pragma unroll`,
// indexing `reg_packed_bins[it]` with the compile-time `it`. NVCC keeps the
// array in registers (it would have spilled with dynamic index).
//
// MAX_ITERS=4 covers T ≥ 256 / length ≤ 4096 (quads / T ≤ 4).
//
...
// Monotonic order-preserving transform on R; rebalances the radix bin
// distribution for clustered score distributions like normal(mean=5,std=1).
//   0 = identity              5 = signed square then mantissa-spread (x*|x| * 2^k)
//   1 = sign-preserving square (x * |x|)
//   2 = cube (x*x*x)
//   3 = x^5
//   4 = x^7
...
__device__ void fast_topk_regcache_unrolled(
    ...
    int           target_k)
{
    ...
    // Compile-time-indexable register cache.
    ...
    // --- Middle vec4 — UNROLLED, fixed-trip loop with constant `it` index ---
    #pragma unroll
    ...
    // --- Middle: cached bins; re-load only when threshold-bin present ---
    #pragma unroll
    for (int it = 0; it < MAX_ITERS; ++it) {
        const int q = tx + it * BLOCK_SIZE;
        if (q < quads) {
            ...
            // First pass: emit strict winners (bin only).
            int bins[4];
            ...
            // Re-load only when at least one element is in threshold bin.
            if (any_threshold) {
                float f0, f1, f2, f3;
    ...
    const int     block_reserved_eos)
{
    ...
    const int topk_val = sparse_kv_indptr[bx + 1] - sparse_kv_indptr[bx] - block_reserved_bos - block_reserved_eos;
    const int nblk  = end - start;
    if (nblk <= topk_val) return;

    const int*    __restrict__ idx_blk   = dense_kv_indices + start;
    int*          __restrict__ out_blk   = sparse_kv_indices + sparse_kv_indptr[bx] + block_reserved_bos;

    __shared__ int s_indices[VORTEX_MAX_TOPK];

    if constexpr (std::is_same_v<ScoreT, __nv_bfloat16>) {
        fast_topk_regcache_unrolled<ScoreT, /*MaxRefineRounds=*/2>(score, s_indices, start, nblk, topk_val);
    } else {
        fast_topk_regcache_unrolled<ScoreT, /*MaxRefineRounds=*/4>(score, s_indices, start, nblk, topk_val);
    }
    __syncthreads();
