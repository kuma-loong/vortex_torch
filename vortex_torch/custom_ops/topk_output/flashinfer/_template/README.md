# `_template/` — copy-and-edit scaffold for a new `topk_output` / `flashinfer` bucket
...
Drop this whole directory under a new bucket name, then edit:
  ...
  1. **`meta.json`**
     ...
     * Replace the `constraints` placeholder with real applicability
       ...
       See `custom_ops/topk_output/SPEC.md` for the kwargs this op routes on.
...
from vortex_torch.custom_ops.topk_output._reference import reference_flashinfer

fn = find("topk_output", "flashinfer", <your routing kwargs>)()  # picks your leaf
out_kernel = ...
out_ref    = ...
