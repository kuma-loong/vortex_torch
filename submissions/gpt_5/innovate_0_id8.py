...
from vortex_torch.indexer import Add, GeMM, Mean, WhereGreater, topK
from vortex_torch.cache import Mean as CMean, L2Norm as CL2Norm


@register("gpt_5_innovate_0_id8_cls")
class HighValueOutlierGate(vFlow):
    def __init__(self):
        super().__init__()
        self.q_mean = Mean(dim=1)
        self.dot = GeMM()
        self.value_energy = GeMM()
        self.avg_energy = Mean(dim=0)
        self.keep_outliers = WhereGreater()
    ...
    def forward_indexer(self, q: torch.Tensor, o: torch.Tensor, cache: Dict[str, torch.Tensor], ctx: ContextBase):
        ...
        mean_energy = self.avg_energy(energy, ctx=ctx)
        gate = self.keep_outliers(energy, mean_energy, ctx=ctx)
