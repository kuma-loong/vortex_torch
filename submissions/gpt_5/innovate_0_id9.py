...
from vortex_torch.indexer import GeMM, Mean, Multiply, Softmax, topK
from vortex_torch.cache import Mean as CMean, L2Norm as CL2Norm


@register("gpt_5_innovate_0_id9_cls")
class SoftmaxValueTemperature(vFlow):
    def __init__(self):
        super().__init__()
        self.q_mean = Mean(dim=1)
        self.dot = GeMM()
        self.softmax_pages = Softmax(dim=0, scale=1.0)
    ...
    def forward_indexer(self, q: torch.Tensor, o: torch.Tensor, cache: Dict[str, torch.Tensor], ctx: ContextBase):
        ...
        prob = self.softmax_pages(raw, ctx=ctx)
        value = self.value_energy(q_mean, cache["v_norm"], ctx=ctx)
        score = self.scale(prob, value, ctx=ctx)
