...
from vortex_torch.indexer import GeMM, Mean, Multiply, Normalize, Softmax, topK
from vortex_torch.cache import Mean as CMean, L2Norm as CL2Norm


@register("gpt_5_innovate_0_id19_cls")
class NormalizedRelevanceValueProduct(vFlow):
    def __init__(self):
        super().__init__()
        self.q_mean = Mean(dim=1)
        self.dot = GeMM()
        self.relevance_prob = Softmax(dim=0, scale=1.0)
        self.value_energy = GeMM()
        self.value_prob = Normalize(dim=0)
    ...
    def forward_indexer(self, q: torch.Tensor, o: torch.Tensor, cache: Dict[str, torch.Tensor], ctx: ContextBase):
        ...
        relevance = self.relevance_prob(raw, ctx=ctx)
        value_energy = self.value_energy(q_mean, cache["v_norm"], ctx=ctx)
        value_weight = self.value_prob(value_energy, ctx=ctx)
        score = self.product(relevance, value_weight, ctx=ctx)
